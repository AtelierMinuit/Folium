...
/// sin abrir la ventana principal de Folium.
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Button {
                ...
                if let window = NSApplication.shared.windows.first(where: { $0.title.contains("Folium") || $0.isKeyWindow }) {
                    window.makeKeyAndOrderFront(nil)
                }
            } label: {
                Label("Abrir Folium", systemImage: "macwindow")
                    .frame(maxWidth: .infinity)
            }
