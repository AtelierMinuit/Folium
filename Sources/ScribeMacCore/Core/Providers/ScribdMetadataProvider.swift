...
public struct ScribdMetadataProvider: DocumentProvider {
    ...
    public func resolveDownload(_ url: URL) async throws -> DownloadResult {
        throw ProviderError.restrictedResource(
            "El documento está clasificado técnicamente como restringido (sin descarga pública autorizada). ScribeMac respeta los términos del servicio y no elude mecanismos de acceso. Para estudiar el pipeline de descarga y procesamiento, utilice un enlace scribemac://fixture/... o un PDF directo."
        )
    }
